...
void gltactics::map_generator::vertical_split(size_t x, size_t y, size_t width, size_t height, size_t hor_door) {
    if (height <= 6) return;
    size_t left_room_width; // separes room into left and right rooms
    size_t w_rand_mod = 2;
    std::uniform_int_distribution<> w_dis(-w_rand_mod, w_rand_mod);
    size_t h_rand_mod = (height - 1);
    ...
    size_t right_room_width = width - left_room_width;
...
void
gltactics::map_generator::horizontal_split(size_t x, size_t y, size_t width, size_t height, size_t vert_door) {
    if (width <= 6) return;
    size_t top_room_height; // separates room into top and bottom rooms
    size_t w_rand_mod = width - 1;
    ...
    size_t bottom_room_height = height - top_room_height;
...
std::vector<gltactics::attribute> gltactics::map_generator::place_chests() {
    std::uniform_int_distribution<size_t> dis(0, DEFAULT_MAPSIZE);
...
void gltactics::map_generator::placeDoors(const std::vector<gltactics::attribute> &doorTypes) {
    ...
    while (idx != doorTypes.size()) {
        for (size_t x = gltactics::map_generator::map_size / 2 + 1; x < gltactics::map_generator::map_size; x++) {
